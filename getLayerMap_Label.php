...
for ($x = 1; $x <= $jumtempisitabel-1; $x++) {
	$sql1="select tipe from ".$tempisitabel[$x]." limit 1 ";
	...
	if ($result1)
	{
		while ($row1=mysqli_fetch_array($result1))
		{
			if ($row1['tipe']=='1')
			{
				$sql="select replace(replace(replace(AsText(ogc_geom),'POINT(',''),')',''),' ',' , ')as geom ,ID, simbol
				from ".$tempisitabel[$x]."  where ogc_geom<>'' ";
			...
			if ($row1['tipe']=='5')
			{
				$sql="select replace(replace(AsText(ogc_geom),'MULTIPOLYGON(((','POLYGON(('),')))','))') as geom, ID,kode as tempkode,fillku,tebalfillku,garisku,tebalgarisku,labelku,teballine  from ".$tempisitabel[$x]."  where ogc_geom<>''  ";
				...
				if ($result)
				{
					...
					while ($row=mysqli_fetch_array($result))
					{
						...
						$garisku=$row['garisku'];
						$tebalgarisku=$row['tebalgarisku'];
						$teballine=$row['teballine'];
						$labelku=$row['labelku'];
						$labelkutampil=get_Isi_Field1($row['labelku'],$tempisitabel[$x],'ID',$kode);
						...
						{
						echo "
						feature.set('description', '".$labelkutampil."');
			...
			if ($row1['tipe']=='3')
			{
				...
				$sql="select replace(replace(AsText(ogc_geom),'MULTILINESTRING(((','LINE(('),')))','))') as geom, ID,kode as tempkode,garisku,tebalgarisku,teballine from ".$tempisitabel[$x]."  where ogc_geom<>'' ";
