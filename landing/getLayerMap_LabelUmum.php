...
$source=$_GET['source'];
$label=$_GET['label'];
$longitude=$_GET['longitudeku'];
$latitude=$_GET['latitudeku'];
?>
	...
	if ($result1)
	{
		while ($row1=mysqli_fetch_array($result1))
		{
			...
			if ($row1['tipe']=='5')
			{
				...
				$sql="select replace(replace(AsText(ogc_geom),'MULTIPOLYGON(((','POLYGON(('),')))','))') as geom, ID_AUTO as ID,kodetabel as tempkode,fillku,tebalfillku,garisku,tebalgarisku,labelku,teballine,PERUNTUKAN  from ".$tempisitabel[$x]." WHERE ST_CONTAINS(ogc_geom, ST_GeomFromText( 'POINT(".$longitude." ".$latitude.")', 4294967295 )) or ST_Distance(ogc_geom, ST_GeomFromText( 'POINT(".$longitude." ".$latitude.")', 4294967295 )) <= 0.0005";
				...
				{
				?>
				alert("Data Layer Tersebut  Terpilih Belum Tersedia..");
				return;
				<?php 
				}
				if ($result)
				{
					...
					while ($row=mysqli_fetch_array($result))
					{
						...
						$labelku=$row['labelku'];
						$labelkutampil=get_Isi_Field1($row['labelku'],$tempisitabel[$x],'ID_AUTO',$kode);
