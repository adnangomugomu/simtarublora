<?php 
	  ...
	  $peruntukkanpermohonantataruang=decrypt($_GET['prt']);
	  $latitudesurvei=decrypt($_GET['lat']);
	  $longitudesurvei=decrypt($_GET['long']);
?>
<!DOCTYPE html>
<html>
<head>
...
<div id="divLoading"></div>
	  ...
	  <div class="row">
        ...
        <div class="col-md-12">
          <!-- Box Comment -->
              <fieldset style="background-color:#eaeaea;padding:20px;border-radius:10px;">
				<div class="col-md-12">
				<label>Pencarian Lokasi:</label>
				</div>
			  	<div class="col-md-3">
                <input class="rounded" id="searchTextField" placeholder="Masukkan nama lokasi yang dicari" type="text" data-toggle="tooltip" data-placement="bottom" title="Masukkan nama lokasi yang dicari">
				</div>
				<div class="col-md-3">
				<input class="rounded" id="latitude" placeholder="Latitude" type="text" size="10" readonly="yes" onKeyPress="return isNumberKey(event)" data-toggle="tooltip" data-placement="bottom" title="Masukkan Latitude lokasi" value="<?php echo $latitudesurvei ?>">
				</div>
				<div class="col-md-3">
			    <input class="rounded" id="longitude" placeholder="Longitude" type="text" size="10" readonly="yes" onKeyPress="return isNumberKey(event)" data-toggle="tooltip" data-placement="bottom" title="Masukkan Longitude lokasi" value="<?php echo $longitudesurvei ?>">
				</div>
				<div class="col-md-3">
			  ...
			  <input class="rounded" id="isiperuntukkan" name="isiperuntukkan" placeholder="Masukkan Peruntukkan Permohonan" type="text" size="10" data-toggle="tooltip" data-placement="bottom" title="Masukkan Peruntukkan Permohonan" value="<?php echo $peruntukkanpermohonantataruang ?>">
			  </p>
			  <br>
			  <p>
			  
			  <input type="hidden" name="koordinatku" id="koordinatku" value="kosong" style="width:100%;display:block;">	
			  ...
			  <select id="type" class="form-control select2" style="width:100%">
				<option value="None">Pilih Model Digitasi</option>
				<option value="area">Area</option>
			  </select>
			  </p>
			  <button type="button" class="btn btn-default" style="background-color:#6699FF;border:none;color:#FFFFFF;font-size:12px;" onClick="klikku6()"><b>Hapus Hasil Digitasi</b></button>
			...
			<div class="box-body" id="map">
			  <div id="mouse-position" style="top:300px; right:50px; z-index:2;position:inherit;color:#FF0000;font-family:'Mangal',Verdana, Arial, Helvetica, sans-serif;"></div>
...
function addMainLayerMap_LabelUmum(mytable,mytampil,mysource,mylat,mylong){
popup.hide();
$.ajax({
	type: "GET",
	url:"landing/getLayerMap_LabelUmum.php",
	data: { table: mytable, tampil: mytampil, source: mysource, latitudeku: mylat, longitudeku: mylong},
	beforeSend:function() {
		document.getElementById("divLoading").style.display = "block"; 
...
if (sketchadi=='mulai')
{
	...
	var f = map.forEachFeatureAtPixel(
        ...
        function(ft, layer)
		{
			...
			{
			//alert('tes');
			getKonten('',xy,document.getElementById("measureOutput").innerHTML,document.getElementById('koordinatku').value);
			}
			else
			{
			getKonten(ft.getId(),xy,hdms,document.getElementById('koordinatku').value);
			}
...
{
?>
document.getElementById("koordinatku").value = "kosong";
<?php
}
else
{
...
$('#radio6').click(function(){
	if (this.checked) {
	  ...
	  parent.document.getElementById('isilongitude').value=lng;
	  parent.document.getElementById('isilatitude').value=lat;
	  addMainLayerMap_LabelUmum('one_peta','rencana_pola_ruang','sourceFeatures',lat,lng);
...
function addInteraction() {
  ...
  draw.on('drawend',
      function(evt) {
		...
		.remove()
		.end()
		.append('<option value="None">Pilih Model Digitasi</option>')
		.append('<option value="area">Area</option>')
		.val('None')
		;
...
function getKonten(str,str1,str2,str3)
  ...
  var url="landing/getKonten3.php";
...
function kontenChanged() { 
    ...
    document.getElementById("peruntukkan").value=xmlHttp.responseText;
	parent.document.getElementById("isiperuntukkan").value=xmlHttp.responseText;
}
</script>
...
{
?>
addMainLayerMap_LabelPermohonan('si_permohonan','<?php echo $_GET['kd']; ?>','sourceFeaturesSurvei');
<?php
}
?>
...
{
?>
addMainLayerMap_LabelUmum('one_peta','rencana_pola_ruang','sourceFeatures','<?php echo $latitudesurvei; ?>','<?php echo $longitudesurvei; ?>');
addMainLayerPoint('','customStyleLokasi','<?php echo $longitudesurvei; ?>','<?php echo $latitudesurvei; ?>');
<?php
}
?>
</script>
