...
$sql="select replace(replace(AsText(ogc_geom),'MULTIPOLYGON(((','POLYGON(('),')))','))') as geom
,SUBSTRING_INDEX(replace(replace(AsText(centroid(ogc_geom)),'POINT(',''),')',''),' ',1) as longi
,SUBSTRING_INDEX(replace(replace(AsText(centroid(ogc_geom)),'POINT(',''),')',''),' ',-1) as lat
, id as ID from ".$table."  where ogc_geom<>'' and deleted='0' and id='".$tampil."' ";
