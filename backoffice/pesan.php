...
{
	$sql="select * from one_pesan where deleted='0' and id='".$kode."' ";
...
if ($action=='h')
{
$sql="update one_pesan set deleted='1' where id='".$kode."'";
//echo $sql;
//exit;
$result=mysqli_query($link,$sql);
?>
<script>
window.location='pesan.php?suc=2';
</script>
<?php 
exit;
}
...
if ($_FILES['ikon']['name'])
        {
          ...
          $gambar = $newfilename;
		  $sql="insert into one_pesan (id,nama,ikon,deleted) values (null,'".$nama."','".$gambar."','0')";
        }
		else
		{
			$sql="insert into one_pesan (id,nama,deleted) values (null,'".$nama."','0')";
		}
...
?>
<script>
window.location='pesan.php?suc=1';
</script>
<?php 
exit;
}
...
if ($_FILES['ikon']['name'])
        {
          ...
          copy ($_FILES['ikon']['tmp_name'], $attachdir . $newfilename);
          $gambar = $newfilename;
		  $sql="update one_pesan set nama='".$nama."',ikon='".$gambar."' where id='".$kodeku."'";
        }
		else
		{
			$sql="update one_pesan set nama='".$nama."' where id='".$kodeku."'";
		}
						...
						?>
							<div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
										<?php
										$sql="select * from one_pesan where deleted='0' ";
										$result=mysqli_query($link,$sql);
										if ($result)
										{
										$i=1;
