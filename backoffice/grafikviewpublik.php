                                ...
                                <div role="tabpanel" class="tab-pane fade in active" id="profile_with_icon_title">
									...
									<p>
										<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
											<canvas id="bar_chart<?php echo $row['id']; ?>" height="150"></canvas>
										</div>
									</p>
	<script>
$(function () {
	try
	{
    new Chart(document.getElementById("bar_chart<?php echo $row['id']; ?>").getContext("2d"), getChartJs<?php echo $row['id']; ?>('bar'));
	}
	catch(err) {
	}
});

function getChartJs<?php echo $row['id']; ?>(type) {

<?php
$sqlcek="select * from one_grafik where id='".$row['id']."' ";
//echo $sqlcek;
...
$fil3=$rowcek['fil3'];
$nama=$rowcek['nama'];
$judulbatang=$rowcek['judulbatang'];
}
}	
?>   
    var config = null;
    ...
    if (type === 'bar') {
        config = {
            ...
            data: {
				...
				[
				<?php
				$sql3="select ".$fil1." from ".$nama." where ".$fil1."<>'' group by ".$fil1."  ";
				//echo $sql3;
                ...
                datasets: [{
					...
					<?php
					$sql3="select count(".$fil2.") as ".$fil2." from ".$nama." where ".$fil1."<>'' group by ".$fil1." ";
					$result3=mysqli_query($link,$sql3);
					if ($result3)
					{
					while ($row3=mysqli_fetch_array($result3))
    ...
    return config;
									...
									?>
									<p>
									Grafik belum di set di halaman administrator..
									</p>
									<?php
									}
									?>
                                ...
                                <div role="tabpanel" class="tab-pane fade" id="settings_with_icon_title">
									...
									<p>
										<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
											<canvas id="pie_chart<?php echo $row['id']; ?>" height="150"></canvas>
										</div>
									</p>
...
$(function () {
	try
	{
    new Chart(document.getElementById("pie_chart<?php echo $row['id']; ?>").getContext("2d"), getChartJspie<?php echo $row['id']; ?>('pie'));
	}
	catch(err) {
	}
});

function getChartJspie<?php echo $row['id']; ?>(type) {
<?php
$sqlcek="select * from one_grafik where id='".$row['id']."' ";
$resultcek=mysqli_query($link,$sqlcek);
if ($resultcek)
        ...
        config = {
            ...
            data: {
                datasets: [{
                    data: [
					<?php
					$sql3="select count(".$fil3.") as ".$fil3." from ".$nama." group by ".$fil3." ";
					$result3=mysqli_query($link,$sql3);
					if ($result3)
					{
					$c=1;
                ...
                labels: [
					<?php
					$sql3="select ".$fil3." from ".$nama." group by ".$fil3." ";
					$result3=mysqli_query($link,$sql3);
					if ($result3)
					{
					while ($row3=mysqli_fetch_array($result3))
